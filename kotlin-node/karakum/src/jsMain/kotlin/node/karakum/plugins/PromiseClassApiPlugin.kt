...
private fun convertAsyncSignature(name: String, propertyType: SignatureDeclarationBase, context: Context, render: Render<Node>): String {
    val typeParameters = propertyType.typeParameters?.asArray()
        ?.joinToString(", ") { render(it) }

    val returnType = propertyType.type ?.let { render(it) }

    val promiseDeclaration = convertParameterDeclarations(
        propertyType, context, render,
    ...
    val type = requireNotNull(propertyType.type)
