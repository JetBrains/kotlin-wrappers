...
external interface MutationOptions<TData, TError, TVariables, TContext> {
    ...
    val onSuccess: ((data: TData, variables: TVariables, context: TContext) -> Promise<Unit>?)?
    val onError: ((error: TError, variables: TVariables, context: TContext?) -> Promise<Unit>?)?
    val onSettled: ((data: TData?, error: TError?, variables: TVariables, context: TContext?) -> Promise<Unit>?)?
