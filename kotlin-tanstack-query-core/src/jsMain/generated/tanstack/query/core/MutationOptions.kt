...
external interface MutationOptions<TData, TError, TVariables, TContext> {
    ...
    val onSuccess: ((data: TData, variables: TVariables, context: TContext) -> Promise<Void>?)?
    val onError: ((error: TError, variables: TVariables, context: TContext?) -> Promise<Void>?)?
    val onSettled: ((data: TData?, error: TError?, variables: TVariables, context: TContext?) -> Promise<Void>?)?
