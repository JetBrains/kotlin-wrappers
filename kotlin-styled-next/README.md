...
downloadCss("some-classname") // download .css file with rules which contains some-classname
getCss("some-classname") // get list of CSS rules which contains some-classname
```

Functions contain optional parameter - part of CSS code used to filter the CSS rules from the DOM

To show all CSS code inside `<style>` element in the DOM call `GlobalStyles.useDevSheet()` in your application before using any CSS 
