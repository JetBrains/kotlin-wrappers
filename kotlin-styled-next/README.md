...
### Debug
To get CSS in styled-next application use `StyledNext.downloadCss` and `StyledNext.getCss` functions:
```js
StyledNext.downloadCss() // download all the CSS generated by application
StyledNext.getCss() // get list of all CSS rules
StyledNext.downloadCss("some-classname") // download .css file with rules which contains some-classname
StyledNext.getCss("some-classname") // get list of CSS rules which contains some-classname
```
Note that these functions are not available in production build.

The functions contain optional parameter - part of the CSS code used to filter the CSS rules from the DOM

To display all the CSS inside styled-next `<style>` element call `GlobalStyles.useDevSheet()` in your application before using any CSS. 
Note that using the `DevSheet` degrades performance.
