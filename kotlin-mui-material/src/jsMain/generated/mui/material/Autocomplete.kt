...
external interface AutocompleteProps<Value> :
    UseAutocompleteProps<Value>, StandardProps, HTMLAttributes<HTMLDivElement>, PropsWithSx {
     ...
     *
     * `Autocomplete` treats `undefined` and `false` differently. If `undefined`, the inner input
     * takes up the full width of its container. If `false`, the inner input is restricted to its
     * intrinsic width.
     *
     * @default false
     */
    ...
    /**
     * If `true`, the component is in a loading state. This shows the `loadingText` in place of
     * suggestions (only if there are no suggestions to show, for example `options` are empty).
     *
     * @default false
     */
    var loading: Boolean?
    ...
    /**
     * The maximum number of tags that will be visible when not focused. Set `-1` to disable the
     * limit.
     *
     * @default -1
     */
     ...
     * Render the input.
     *
     * **Note:** The `renderInput` prop must return a `TextField` component or a compatible custom
     * component that correctly forwards `InputProps.ref` and spreads `inputProps`. This ensures
     * proper integration with the Autocomplete's internal logic (e.g., focus management and
     * keyboard navigation).
     *
     * Avoid using components like `DatePicker` or `Select` directly, as they may not forward the
     * required props, leading to runtime errors or unexpected behavior.
     *
     * @param {object} params
     * @returns {ReactNode}
     */
    ...
    var renderOption:
            ((
                ...
                ownerState: AutocompleteOwnerState<Value>,
            ) -> ReactNode)?

    /**
     * Renders the selected value(s) as rich content in the input for both single and multiple
     * selections.
     *
     ...
     */
    var renderValue:
            ((value: Any, getItemProps: Any, ownerState: AutocompleteOwnerState<Value>) -> ReactNode)?
...
external interface AutocompleteRenderInputParams : Props {
    ...
    interface SlotProps {
        var inputLabel:
                Props? /* ReturnType<ReturnType<typeof useAutocomplete>['getInputLabelProps']> */
        var input: Any?
