...
external interface UseVirtualizerOptions<TScrollElement : web.dom.Element, TItemElement : web.dom.Element> {
    val count: Int
    val getScrollElement: () -> TScrollElement?
    val estimateSize: (index: Int) -> Int
    val scrollToFn: (
        (offset: Int, options: ScrollOptions, instance: Virtualizer<TScrollElement, TItemElement>) -> Unit
    )?
    val observeElementRect: (
        (instance: Virtualizer<TScrollElement, TItemElement>, cb: (rect: Rect) -> Unit) -> (() -> Unit)?
    )?
    val observeElementOffset: (
        (instance: Virtualizer<TScrollElement, TItemElement>, cb: ObserveOffsetCallBack) -> (() -> Unit)?
    )?
    val debug: Boolean?
